...
# ╔═╡ 777c3c04-ba16-11ef-1dd1-dbdb05aeec5b
using FastaIO, Plots, PlotlyJS, PlutoUI, CSV, DataFrames

# ╔═╡ 53471d45-5725-45bf-90d6-979fd51437d8
md"""
# Specifying dependencies
"""

# ╔═╡ 15a4acc4-b657-4463-adb2-ccc3d36f5c71
plotlyjs()

# ╔═╡ 18a50cc4-ef98-48bc-bdc2-4fe2a129ea10
TableOfContents()

# ╔═╡ 88713e90-f534-4021-b95a-72ced6966078
md"""
# Loading data (into RAM)
"""

# ╔═╡ c19c423e-8b08-457c-b4db-44846b1f9ecf
fasta_file = "DATA/GCF_017654675.1_Xenopus_laevis_v10.1_genomic.fna"

# ╔═╡ a3c62f75-2f45-4ce3-8da1-5acb90ef7b90
bed_file = "DATA/cpg.bed"

# ╔═╡ c977be2c-eff5-4cd7-9bab-3d3b2fdde986
genome = [(header, uppercase(seq)) for (header, seq) in FastaReader(fasta_file)]

# ╔═╡ e133f386-cff8-45d0-ab63-4c2dbf8cde59
chrom = genome[5] # taken arbitrary

# ╔═╡ bc24d6c3-aef4-4089-b439-5c09a3982848
md"""
# Visualizing dataset
"""

# ╔═╡ 1979109c-511b-4f44-afd9-db8bad5954c1
md"""
## Declaring functions
"""

# ╔═╡ 53773348-8e89-4843-a026-b1585f541012
"""Fancy bioseq length formatting"""
...
# ╔═╡ 0ad50bce-d77f-4567-9150-504b71d633c5
"""Letters in sequence as scatterplot"""
function plot_dna_sequence(seq, slider_pos, window_length=300)
	_deduce_start_pos(pos::Float64) = max(min(round(Int, 0.01L * pos), L-window_length), 1)
	_deduce_start_pos(pos::Int64) = max(min(pos, L-window_length), 1)
	
	dna = seq[2]
	L = length(dna)
	start_pos = _deduce_start_pos(slider_pos)
	end_pos = min(L, start_pos+window_length)

	v = @view dna[start_pos:end_pos]
	x_A = findall('A', v) .+ (start_pos-1)
	x_G = findall('G', v) .+ (start_pos-1)
	x_C = findall('C', v) .+ (start_pos-1)
	x_T = findall('T', v) .+ (start_pos-1)
	x_N = findall('N', v) .+ (start_pos-1)
	ticks_step = window_length ÷ 6
    p = Plots.plot(
		title="$(bioseq_readlen(L)), $(join(split(seq[1])[2:min(7, end)], ' '))",
		titlefontsize=10,
		legend=false,
		ylims=(0,6),
		grid=false,
		xticks=(start_pos:ticks_step:end_pos, 5),
		yticks=([1, 2, 3, 4, 5], ["A", "G", "C", "T", "N"]),
		size=(1450, 300),
	)

    scatter!(p, x_A, fill(1, length(x_A)), ms=1.2, mc="#5050FFFF", msw=0, label="A")
    scatter!(p, x_G, fill(2, length(x_G)), ms=1.2, mc="#00C000FF", msw=0, label="G")
    scatter!(p, x_C, fill(3, length(x_C)), ms=1.2, mc="#E00000FF", msw=0, label="C")
    scatter!(p, x_T, fill(4, length(x_T)), ms=1.2, mc="#E6E600FF", msw=0, label="T")
    scatter!(p, x_N, fill(5, length(x_N)), ms=1.2, mc="#AE00FEFF", msw=0, label="N")
	p
end

# ╔═╡ 6648bc83-6bff-438f-b2a7-55192733b892
"""Local frequency of `N` nucleotides plot"""
function plot_local_frequency(seq, window_size::Int)
	header, dna = seq
    @assert window_size > 0 && window_size <= length(dna) "Window size must be greater than 0 and less than or equal to the length of the sequence."
	plot_range = 1:window_size:(length(dna) - window_size + 1)
    coordinates = collect(plot_range)
    frequencies = [
		count('N', @view dna[i:i+window_size-1]) / window_size
		for i in plot_range
	]
    Plots.plot(
		coordinates, frequencies,
		ylabel="Frequency of 'N'",
		title="$(join(split(header)[2:min(7, end)], ' ')) local frequency of 'N' within $(bioseq_readlen(window_size)) windows", titlefontsize=10,
		size=(1450, 300), grid=false,
		xticks=(range(extrema(plot_range)..., 10), 0),
		legend=false,
	)
end

# ╔═╡ 056a896a-245f-41ff-9fbb-003518a9e31b
"""Lengths of `N`-regions histogram"""
function plot_n_repeating_regions_histogram(dna_sequence::String)
	function n_repeating_regions(dna_sequence::String)
		# length.(findall(r"N+", dna_sequence)) but faster
	    lengths = Int[]
	    count = 0
    ...
    lengths = n_repeating_regions(dna_sequence)
    Plots.histogram(lengths,
		xlabel="Length of N-repeating regions",
		ylabel="Counts",
		title="Histogram of N-repeating regions",
		legend=false,
		bin=40,
		xticks=(0, 0),
	)
...
# ╔═╡ eac925a4-5570-481b-a91e-7be4a90a4a48
md"""
## Plots
"""

# ╔═╡ dc624cf7-7b37-4834-8d58-415338aa54b5
html"""
<style>
input[type*="range"] {
	width: 100%;
...
# ╔═╡ 4f815b00-d681-481a-9016-4b09910d37a5
md"""
$(@bind slider_pos Slider(0:0.01:100, show_value=true))
"""

# ╔═╡ 0e28028e-8b50-49ca-a5a0-b957fd11c691
plot_dna_sequence(chrom, 9_378_700, 6000)#findfirst('N', genome[5][2]) - 300, 700)

# ╔═╡ 252f043a-78ab-4016-9bd9-7458c4ad917f
plot_local_frequency(chrom, 10000)

# ╔═╡ e748834e-e42a-46f1-b658-dc30940619d5
plot_n_repeating_regions_histogram(chrom[2])

# ╔═╡ 8e95803f-621b-4c3c-b9de-27945e97cfb5
md"""
# Pruning dataset
"""

# ╔═╡ 73695080-221a-4357-8f44-0cbd81268945
md"""
## Declaring functions
"""

# ╔═╡ 4eeb7610-d2c6-46ba-85e1-baf8a21bacd1
...
# ╔═╡ 420b600e-4cff-4991-a097-d6a8fdefb6e7
...
# ╔═╡ b2c3e32c-707c-4ab8-90f4-5a2522de0e3d
"""save bed data as tsv"""
function save_bed_to_disk(bed_df::DataFrame, filename::String)
...
# ╔═╡ 75ff7ce2-b06f-415b-a32a-92266e1cd3fd
...
# ╔═╡ c67f47fe-348d-4bf6-9d3b-486c7bb1d954
...
# ╔═╡ 05da6460-327f-4a0e-9ea0-45ec9a46f9fc
md"""
## Actually applying functions to data
"""

# ╔═╡ b173e257-f273-4d8b-bf5f-e5a401ba94f0
bed_df = parse_bed_file(bed_file)

# ╔═╡ 06e7a304-94e9-4e31-8d35-dde31cad957d
filtered_bed, filtered_genome = filter_ns(bed_df, genome)

# ╔═╡ 4770806f-1ae5-457e-8dc3-eb8aca6c263d
md"""
## Save to disc if needed
"""

# ╔═╡ 670f6b9a-9582-4771-89df-6bcb30ff72fa
# ╠═╡ disabled = true
#=╠═╡
save_bed_to_disk(filtered_bed, "cpg_pruned.bed")
  ╠═╡ =#

# ╔═╡ 8edd921d-fcee-4261-b656-c7b9ce854cd5
# ╠═╡ disabled = true
#=╠═╡
writefasta("genome_pruned.fa", filtered_genome; check_description=false)
  ╠═╡ =#
...
manifest_format = "2.0"
project_hash = "a6a56b1a824c3d654d8e03e3bdfca9eea5a0c604"
...
[[deps.FastaIO]]
deps = ["GZip"]
git-tree-sha1 = "0133228b2353194fe0703d8cba182f832e11bb54"
